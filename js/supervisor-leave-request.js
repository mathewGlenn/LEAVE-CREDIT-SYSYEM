// Supervisor Leave Request Functionality for LCMS
// Uses Firebase compat SDK for browser compatibility
// Supervisors skip supervisor approval and go directly to HR approval

class SupervisorLeaveRequestManager {
    constructor() {
        this.uploadedFiles = [];
    ...
    validateFile(file) {
        ...
        if (!this.allowedTypes.includes(file.type)) {
            showErrorAlert('Invalid File Type', `File type ${file.type} is not allowed. Please upload PDF, DOC, DOCX, JPG, or PNG files.`);
            return false;
        }

        // Check file size
        if (file.size > this.maxFileSize) {
            showErrorAlert('File Too Large', `File ${file.name} is too large. Maximum size is 10MB.`);
            return false;
        }
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            // Create leave request document with supervisor approval already approved
            const leaveRequest = {
                employeeId: user.uid,
                ...
                status: 'pending_hr', // Supervisors skip supervisor approval and go directly to HR
                supervisorApproval: {
                    status: 'approved', // Auto-approved since they are supervisor
                    approvedBy: user.uid,
                    approvedAt: firebase.firestore.FieldValue.serverTimestamp(),
                    comments: 'Auto-approved - Supervisor request'
                },
                hrApproval: {
                    status: 'pending',
                    approvedBy: null,
            ...
            showSuccessAlert('Request Submitted', 'Leave request submitted successfully! Your request has been automatically approved by supervisor and is now pending HR approval.');
...
// Initialize the supervisor leave request manager when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    new SupervisorLeaveRequestManager();
});
